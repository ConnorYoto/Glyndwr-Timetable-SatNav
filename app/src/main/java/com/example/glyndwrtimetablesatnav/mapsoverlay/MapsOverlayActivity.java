...
public class MapsOverlayActivity extends FragmentActivity implements LocationListener, OnMapReadyCallback
{
    ...
    private void showBlueDot(LatLng center, double accuracyRadius, double bearing) {
        if (mCircle == null) {
            ...
            if (mMap != null) {
                mCircle = mMap.addCircle(new CircleOptions().center(center).radius(accuracyRadius).fillColor(0x201681FB)
                        .strokeColor(0x500A78DD).zIndex(1.0f).visible(true).strokeWidth(5.0f));
                mMarker = mMap.addMarker(new MarkerOptions().position(center)
                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.map_blue_dot)).anchor(0.5f, 0.5f)
                        .rotation((float) bearing).flat(true));
            }
        } else {
    ...
    private IALocationListener mListener = new IALocationListenerSupport() {   //  Listener that handles location change events.
        @SuppressLint("LogNotTimber")
        @Override
        public void onLocationChanged(IALocation location) {   //  Location changed, move marker and camera position.
            Log.d(TAG, "new location received with coordinates: " + location.getLatitude() + "," + location.getLongitude());
            if (mMap == null) {   // location received before map is initialized, ignoring update here
                return;
            }
    ...
    private IARegion.Listener mRegionListener = new IARegion.Listener() {   //  Listener that changes overlay if needed
        @Override
        public void onEnterRegion(IARegion region) {
            if (region.getType() == IARegion.TYPE_FLOOR_PLAN) {
                final String newId = region.getId();
                if (mGroundOverlay == null || !region.equals(mOverlayFloorPlan)) {   //  // Are we entering a new floor plan or coming back the floor plan we just left?
        ...
        public void onExitRegion(IARegion region) {
            if (mGroundOverlay != null) {   // Indicate we left this floor plan but leave it there for reference
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mMap.setOnMapLongClickListener(new GoogleMap.OnMapLongClickListener()
        {
            ...
            {
                ExampleUtils.shareText(MapsOverlayActivity.this, mIALocationManager.getExtraInfo().traceId, "traceId");
            }
        }); //  mMap.setOnMapLongClickListener(new GoogleMap.OnMapLongClickListener()
    }
