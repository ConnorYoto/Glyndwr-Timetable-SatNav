...
{   //  Contains a primitive shape that can be drawn using GLES20.glDrawArrays
    ...
    GLPrimitive(float [] coordinates, float [] texCoords)
    {
        ...
        {
            throw new IllegalArgumentException("coordinates.length not dividable by three (" + coordinates.length + ")");
        }
        if (texCoords.length % 2 != 0)
        {
            throw new IllegalArgumentException("texCoords.length not dividable by two (" + texCoords.length + ")");
        }
