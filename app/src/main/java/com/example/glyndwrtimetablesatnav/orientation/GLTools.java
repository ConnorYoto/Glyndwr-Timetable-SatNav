...
public class GLTools
{
    ...
    private static final String sVertexSimple = "" + "uniform    mat4        uMatrix;" + "attribute  vec4        vPosition;" +
                    "void main() {" + "  gl_Position = uMatrix * vPosition;" + "}";
    private static final String sFragmentSimple = "" + "precision mediump float;" + "uniform vec4 uColor;" +
                    "void main() {" + "  gl_FragColor = uColor;" + "}";

    // Shader program for textured surfaces
    public static int sProgTexture;
    private static final String sVertexTexture = "" + "uniform    mat4        uMatrix;" + "attribute  vec4        vPosition;" +
                    "attribute  vec2        aTexCoordinate;" + "varying    vec2        vTexCoordinate;" + "void main() {" +
                    "  gl_Position = uMatrix * vPosition;" + "  vTexCoordinate = aTexCoordinate;" + "}";
    private static final String sFragmentTexture = "" + "precision mediump float;" + "uniform    sampler2D   uTexture;" +
                    "varying    vec2        vTexCoordinate;" + "void main() {" + "  gl_FragColor = texture2D(uTexture, vTexCoordinate);" + "}";
    ...
    {   //  Creates a quaternion with the given vector part and zero real part. Needed in quaternion rotations.
